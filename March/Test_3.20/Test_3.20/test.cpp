...
public:
    string minWindow(string s, string t) 
    {
        ...
        {
            hash1[s[right]]++;
            if (hash2.count(s[right]) && hash1[s[right]] <= hash2[s[right]])
                count++;
            while (count == t.size())
            {
                hash1[s[left]]--;

                if (hash2.count(s[left]) && hash1[s[left]] < hash2[s[left]])
                {
                    if (count == t.size())
                    {
                        int tmp = len;
                        len = min(tmp, right - left + 1);
                        if (len != tmp)
                            minRight = left;
                    }
                    ...
                    count--;
                    left++;
                    while (left <= right && !(hash2.count(s[left]) && hash1[s[left]] <= hash2[s[left]]))
                    {
                        hash1[s[left]]--;
                        left++;
                        isOK = true;
        ...
        {
            string tmp(s.begin() + minRight, s.begin() + minRight + len);
            return tmp;
        }
    }
};
